...
const preencherDataGridView = (endpoint) => {
  dados.innerHTML = "";
...
preencherDataGridView('http://localhost:3000/agendas')
...
btn_filtrar.addEventListener('click', (evt) => {
  console.log(f_filtroNome.value, 'f_filtroNome.value')
  if(f_filtroNome.value == ''){
    preencherDataGridView('http://localhost:3000/agendas')
  }else{
    const value_input_pesq = f_filtroNome.value
    preencherDataGridView(`http://localhost:3000/agendas?nome_like=${value_input_pesq}`)
